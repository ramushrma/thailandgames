...
$ab="SELECT * FROM `aviator_results` WHERE `game_sr_num`='$game_sr'";
$query = mysqli_query($conn,$ab);
if(mysqli_num_rows($query)=='0')
{

  $q = "INSERT INTO `aviator_results`(`color`, `game_sr_num`, `game_id`, `price`, `status`, `created_at`,`updated_at`) VALUES ('Aviator','$game_sr','5','$multiplier','1','$datetime','$datetime')";
  $query = mysqli_query($conn,$q);

   if($query){
	   $get_bet_his = "SELECT * FROM `aviator_bets` where `game_sr_num` = '$game_sr' && `game_id`= 5 && `status` = 0 && `win`=0 && `result_status` = 0";
	   ...
	   if($up_query){
		 ...
		 $update = mysqli_query($conn,"UPDATE `aviator_bets` SET `status` = 1,`result_status`=1,`win` = '$win_amount' where `id` = $id");
 ...
 $users_update = mysqli_query($conn,"UPDATE `users` set `wallet` = '$wallet',`winning_wallet` = '$winning' where `id` = '$uid'");
					   ...
					   $update = "UPDATE `aviator_bets` SET `status` = 2,`result_status`= 1 where `id` = $id && `game_sr_num` = '$game_sr'";

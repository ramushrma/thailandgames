...
public function aviatorBet(Request $request){
    ...
    if (!$user) {
        return response()->json(['status' => 400, 'message' => 'Failed to get user data!'], 200);
    }
    ...
    if ($user->bonus >= $amount) {
        // Full deduction from bonus
        $user->bonus -= $amount;
    } else {
        // Deduct what we can from bonus and the rest from wallet
        $remainingAmount = $amount - $user->bonus;
        $user->bonus = 0;
